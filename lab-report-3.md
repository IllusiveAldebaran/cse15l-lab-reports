...
For this lab report we will be focusing on grep. By far it is the one with more that I don't know and more to offer that than `less` or `find`, though I think `find` and some of it's options may be useful to learn later. But grep is useful in many contexts. And I need to review it. Although I know a few flags I will look at three examples of flags for grep *-w, -e, -f*. I will also list out some useful ones but the less work I have to type out, format, and paste pictures when the output is nicer.
